...
/**
 * Seeds one demo tenant ("Gujarat Solar Co.") with a warehouse, a priced
 * product catalog (with B2B volume tiers), and the products the parametric
 * BOM engine matches on by category + rating — enough to exercise every
 * module (D2C calculator, CRM, inventory, B2B, vendor) end to end.
 * Run with: pnpm --filter @solaros/db exec tsx src/seed.ts
 */
async function seed() {
  const pool = new Pool({ connectionString: process.env.DATABASE_URL });

  const tenantRes = await pool.query<{ id: string }>(
    `INSERT INTO tenants (name, slug, subscription_tier, subscription_status, db_mode)
     VALUES ('Gujarat Solar Co.', 'gujarat-solar', 'wholesaler_pro', 'active', 'shared_cluster')
  ...
  const warehouseRes = await pool.query<{ id: string }>(
    `INSERT INTO warehouses (tenant_id, name, city, state, pincode, is_default)
     VALUES ($1, 'Ahmedabad Main WH', 'Ahmedabad', 'Gujarat', '380001', true)
     RETURNING id`,
    [tenantId]
  );
  ...
  console.log(`Seeded tenant "gujarat-solar" (${tenantId}) with warehouse ${warehouseId} and ${products.length} products.`);
  await pool.end();
}
